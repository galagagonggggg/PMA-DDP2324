...
def date_passed(todays_date, scheduled_date):
    ...
    today_list = re.split(" |th|st|rd|nd",todays_date)
    scheduled_list = re.split(" |th|st|rd|nd",scheduled_date)
    month_list = ["January","Febuary","March","April","May","June","July","August","September","October","November","December"]
    today_month = month_list.index(today_list[2])
    scheduled_month = month_list.index(scheduled_list[2])
    result = ""
    if today_month < scheduled_month:
        result = "is yet to pass"
    elif today_month > scheduled_month:
        result = "has passed"
    else:
        if int(today_list[0]) < int(scheduled_list[0]):
            result = "is yet to pass"
        elif int(today_list[0]) == int(scheduled_list[0]):
            result = "is today"
        else:
            result = "has passed"
    return print('Expected: Scheduled date',result)
# Test cases
